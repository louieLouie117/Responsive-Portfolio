const MyInfo = require("../../models/myInfo/myInfo.models");
...
// CRUD operations
    ...
    create (req, res){
        ...
        MyInfo.create(req.body)
        .then(newMyInfo => {
            res.json({MyInfo: newMyInfo})
    ...
    // delete 
    delete (req, res){
        MyInfo.deleteOne({_id: req.params.id})
        .then(result => res.json({MyInfo: result}))
        .catch((err) => {res.status(400).json(err);})
    },
    ...
    // find one 
    findSingleMyInfo (req, res){
        MyInfo.findOne(({_id: req.params.id}))
        .then(oneSingleMyInfo => res.json({MyInfo: oneSingleMyInfo}))
        .catch((err) => {res.status(400).json(err);})
    },
    ...
    // update 
    update (req, res){
        MyInfo.findOneAndUpdate({_id: req.params.id}, req.body)
        .then(result => res.json({MyInfo: result}))
        .catch((err) => {res.status(400).json(err);})
    },
